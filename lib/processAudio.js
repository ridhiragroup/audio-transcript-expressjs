    ...
    try {
        ...
        if (requestBody && typeof requestBody === 'object') {
            if (requestBody.Call_Record_ID) {
                ...
                console.log(`✅ [${requestId}] Found form-data format with Call_Record_ID (will fetch from Voice_Recording__s)`);
            } else {
        ...
        console.log(`🎯 [${requestId}] Processing ID: ${Call_Record_ID}`);
        if (!Call_Record_ID) throw new Error(`Missing required field: Call_Record_ID. Received: ${JSON.stringify(requestData)}`);

        // Always fetch recording URL from Voice_Recording__s field (ignore Call_Recording_URL)
        console.log(`🔄 [${requestId}] Fetching recording URL from Voice_Recording__s field...`);
        const recordingUrl = await getRecordingUrlFromFallback(Call_Record_ID, requestId);
        console.log(`✅ [${requestId}] Successfully obtained recording URL: ${recordingUrl}`);
